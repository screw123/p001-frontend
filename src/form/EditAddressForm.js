...
class EditAddressForm extends React.Component {
	...
	validate(v) {
		const validateFunc = {
			legalName: ({ legalName }) =>
				legalName.length > 0 && legalName.length < 255
					? undefined
					: 'Please provide a valid personal/company name',
			streetAddress: ({ streetAddress }) =>
				streetAddress.length > 4 && streetAddress.length < 500
					? undefined
					: 'Please provide a valid address',
			addressRegion1: ({ addressRegion1 }) =>
				addressRegion1.length > 0 && addressRegion1.length < 50
					? undefined
					: 'Please provide a valid region',
			addressRegion2: ({ addressRegion2 }) =>
				addressRegion2.length > 0 && addressRegion2.length < 50
					? undefined
					: 'Please provide a valid region',
			addressCountry: ({ addressCountry }) =>
				addressCountry && addressCountry.length > 0
					? undefined
					: 'Please choose address Country',
			...
			telephone: ({ telephone }) =>
				telephone.length == 8 ? undefined : 'Please provide a valid phone number'
		}
					...
					<LocaleApiSubscriber>
						{c => (
							<ApolloProvider client={g.getGqlClient()}>
								<Mutation
									mutation={
										this.props.mode === 'add' ? addAddress : updateAddress
									}
									errorPolicy='all'
								>
									{(mutate, { loading, err }) => {
													...
													onSubmit={async (values, actions) => {
														...
														try {
															let vars = {
																...
																addressRegion1:
																	values.addressRegion1,
																addressRegion2:
																	values.addressRegion2,
															...
															if (values.setDefaultBilling) {
																vars['setDefaultBilling'] = true
															}
															if (values.setDefaultShipping) {
																vars['setDefaultShipping'] = true
															}
															...
															<FormikForm align='center'>
																...
																{!get(
																	...
																	<Field
																		...
																		checked={
																			values.setDefaultBilling ===
																			true
																		}
																		err={
																			errors.setDefaultBilling
																		}
																	>
																		<ClickableText>
																			{c.t(
																				'Set this as default billing address'
																			)}
																		</ClickableText>
																	</Field>
																)}
																{!get(
																	...
																	<Field
																		...
																		checked={
																			values.setDefaultShipping ===
																			true
																		}
																		err={
																			errors.setDefaultShipping
																		}
																	>
																		<ClickableText>
																			{c.t(
																				'Set this as default shipping address'
																			)}
																		</ClickableText>
																	</Field>
																)}
